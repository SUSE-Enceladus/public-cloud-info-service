    ...
    with mock.patch('app.assert_valid_provider'):
        if category == 'images':
            with mock.patch('app.get_provider_images',
                ...
                assert rv.headers['Access-Control-Allow-Origin'] == '*'
        if category == 'servers':
            with mock.patch('app.get_provider_servers',
...
def test_get_provider_servers_by_type(client, server_type):
    ...
    with mock.patch('app.assert_valid_provider'):
        ...
        with mock.patch('app.get_provider_servers_for_type',return_value=provider_servers) \
...
def test_get_provider_invalid_server_type(client):
    ...
    provider = 'amazon'
    with mock.patch('app.assert_valid_provider'):
        with mock.patch('app.get_provider_servers_for_type'):
...
def test_provider_images_by_state(client, image_state):
    ...
    with mock.patch('app.assert_valid_provider'):
        with mock.patch('app.get_provider_images_for_state',
